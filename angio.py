...
EVENT_VIEW_COLUMNS = {
    "A": [
        "first_physician", "system_name", "accession_number", "irradiation_event_id",
        "date_time_started", "acquisition_protocol", "dose_area_product", "dose_rp",
        "irradiation_duration", "field_area_cm2", "estimated_field_area_at_sod_cm2",
        "estimated_entrance_field_area_cm2", "field_usage_percent", "collimation_change_flag",
    ],
    "B": [
        "first_physician", "system_name", "accession_number", "irradiation_event_id",
        "irradiation_event_uid", "date_time_started", "acquisition_protocol",
        "dose_area_product", "dose_rp", "irradiation_duration", "pulse_rate",
        "number_of_pulses", "patient_equivalent_thickness", "positioner_primary_angle",
        "positioner_secondary_angle", "distance_source_to_isocenter",
        "distance_source_to_detector", "distance_source_to_entrance_surface",
        "collimated_field_width", "collimated_field_height", "field_area_cm2",
        "estimated_field_area_at_sod_cm2", "fov_mm", "fov_side_cm", "max_fov_area_cm2",
        "field_usage_percent", "collimation_change_score", "collimation_change_flag",
...
SUMMARY_VIEW_COLUMNS = {
    "A": [
        "first_physician", "system_name", "n_events", "mean_dap", "mean_dose_rp",
        "mean_irradiation_duration", "mean_field_area_cm2",
        "mean_estimated_field_area_at_sod_cm2", "mean_estimated_entrance_field_area_cm2",
        "mean_field_usage_percent", "n_flagged_collimation_change_events",
    ],
    "B": [
        "first_physician", "system_name", "n_events", "total_dap", "mean_dap",
        "total_dose_rp", "mean_dose_rp", "total_irradiation_duration",
        "mean_irradiation_duration", "mean_pulses", "mean_pulse_rate",
        "mean_field_area_cm2", "max_field_area_cm2", "mean_estimated_field_area_at_sod_cm2",
        "max_estimated_field_area_at_sod_cm2", "mean_estimated_entrance_field_area_cm2",
        "max_estimated_entrance_field_area_cm2", "mean_field_usage_percent",
        "max_field_usage_percent", "n_flagged_collimation_change_events",
    ],
    "C": None,
}
...
def remove_empty_columns(headers, rows):
    ...
    for idx, header in enumerate(headers):
        has_value = any(idx < len(row) and row[idx] not in (None, "") and str(row[idx]).strip() != "" for row in rows)
        if has_value:
...
def filter_events_by_accession(df, search_text):
    ...
    if not terms:
        return df
    mask = df["accession_number"].fillna("").astype(str).str.lower().apply(lambda v: any(t in v for t in terms))
    return df[mask].copy()
...
def build_summary_from_df(df):
    ...
    first_physician = next((v for v in df.get("first_physician", pd.Series(dtype=object)).tolist() if str(v).strip()), "")
    system_name = next((v for v in df.get("system_name", pd.Series(dtype=object)).tolist() if str(v).strip()), "")

    n_flagged = 0
    if "collimation_change_flag" in df.columns:
        n_flagged = int(df["collimation_change_flag"].fillna("").astype(str).isin(["REVIEW", "LIKELY"]).sum())
...
def run_audit_query(config, date_from, date_to, system_name, physician_name):
    return run_query(
        ...
        enriched AS (
            SELECT
                *,
                CASE WHEN fov_mm IS NOT NULL AND fov_mm > 0 THEN ROUND(fov_mm / 10.0, 4) ELSE NULL END AS fov_side_cm,
                CASE
                ...
                CASE
                    WHEN field_area_cm2 IS NOT NULL AND fov_mm IN (480, 420, 320, 220, 160, 110)
                    THEN ROUND(
                ...
                CASE
                    ...
                    THEN ROUND(
                        field_area_cm2::numeric * POWER(distance_source_to_isocenter::numeric / distance_source_to_detector::numeric, 2),
                        4
                    )
                    ELSE NULL
                ...
                CASE
                    ...
                    THEN ROUND(
                        field_area_cm2::numeric * POWER(distance_source_to_entrance_surface::numeric / distance_source_to_detector::numeric, 2),
                        4
                    )
                    ELSE NULL
        ...
        scored AS (
            SELECT
                e.*, 
                LAG(date_time_started) OVER (PARTITION BY first_physician, system_name ORDER BY date_time_started, irradiation_event_id) AS prev_time,
                LAG(acquisition_protocol) OVER (PARTITION BY first_physician, system_name ORDER BY date_time_started, irradiation_event_id) AS prev_protocol,
                LAG(field_area_cm2) OVER (PARTITION BY first_physician, system_name ORDER BY date_time_started, irradiation_event_id) AS prev_field_area_cm2,
                LAG(positioner_primary_angle) OVER (PARTITION BY first_physician, system_name ORDER BY date_time_started, irradiation_event_id) AS prev_primary_angle,
                LAG(positioner_secondary_angle) OVER (PARTITION BY first_physician, system_name ORDER BY date_time_started, irradiation_event_id) AS prev_secondary_angle,
                LAG(fov_mm) OVER (PARTITION BY first_physician, system_name ORDER BY date_time_started, irradiation_event_id) AS prev_fov_mm
            FROM enriched e
        ),
        final AS (
            SELECT
                *,
                (
                    ...
                    CASE WHEN field_usage_percent IS NOT NULL AND field_usage_percent <= 30 THEN 1 ELSE 0 END +
                    CASE
                        WHEN prev_time IS NOT NULL
                         ...
                         AND ABS(field_area_cm2 - prev_field_area_cm2) / GREATEST(field_area_cm2, prev_field_area_cm2) >= 0.30
                        THEN 2 ELSE 0
                    END
...
def main():
    st.set_page_config(page_title="OpenREM RF Physician Dose Audit", layout="wide")
    st.title("OpenREM RF Physician Dose Audit")
    st.caption("Streamlit edition: database-driven RF event audit with physician summary, event filtering, and CSV export.")
    ...
    if "events_df_all" not in st.session_state:
        st.session_state.events_df_all = pd.DataFrame()
    if "summary_df_all" not in st.session_state:
        st.session_state.summary_df_all = pd.DataFrame()

    with st.sidebar:
        st.subheader("Database settings")
        db_host = st.text_input("Host", value=defaults["host"])
        db_port = st.text_input("Port", value=defaults["port"])
        db_name = st.text_input("Database", value=defaults["dbname"])
        db_user = st.text_input("User", value=defaults["user"])
        ...
        with c1:
            if st.button("Save settings", use_container_width=True):
                ...
                st.success("Connection settings saved. Password not saved.")
        with c2:
            if st.button("Test connection", use_container_width=True):
                try:
                    conn = get_connection(config["dbname"], config["user"], config["password"], config["host"], config["port"])
                    with conn.cursor() as cur:
        ...
        date_from = st.text_input("Date from", value="2025-01-01")
        date_to = st.text_input("Date to", value="2025-12-31")
        view_mode = st.radio("View mode", ["A", "B", "C"], index=1, horizontal=True)

        valid_dates = validate_date(date_from) and validate_date(date_to) and date_from <= date_to
        if not valid_dates:
            st.warning("Use valid YYYY-MM-DD dates and ensure Date from ≤ Date to.")

        if st.button("Load systems", use_container_width=True, disabled=not valid_dates):
            try:
                st.session_state.systems = get_systems(config, date_from, date_to)
                st.session_state.physicians = []
                st.success(f"Loaded {len(st.session_state.systems)} systems.")
            except Exception as exc:
                st.error(str(exc))

        system_name = st.selectbox("System", options=st.session_state.systems, index=None, placeholder="Select a system")

        if st.button("Load physicians", use_container_width=True, disabled=not (valid_dates and system_name)):
            try:
                st.session_state.physicians = get_physicians(config, date_from, date_to, system_name)
                st.success(f"Loaded {len(st.session_state.physicians)} physicians.")
            except Exception as exc:
                st.error(str(exc))

        physician_name = st.selectbox("Physician", options=st.session_state.physicians, index=None, placeholder="Select a physician")

        if st.button("Run audit", type="primary", use_container_width=True, disabled=not (valid_dates and system_name and physician_name)):
            try:
                with st.spinner("Running audit..."):
                    event_headers, event_rows = run_audit_query(config, date_from, date_to, system_name, physician_name)
                    events_df_all = pd.DataFrame(event_rows, columns=event_headers)
                    st.session_state.events_df_all = events_df_all
                    st.session_state.summary_df_all = build_summary_from_df(events_df_all)
    ...
    if events_df_all.empty:
        st.info("No results yet. Configure the database, load systems and physicians, then run the audit.")
        return
    ...
    st.caption("Color key: green < 50%, orange 50-79.99%, red ≥ 80%. Heuristic flags: REVIEW = score 2-3, LIKELY = score ≥ 4.")

    tabs = st.tabs(["Physician summary", "Event rows"])

    with tabs[0]:
        summary_search = st.text_input("Search summary rows", value="")
        summary_headers, summary_rows = filter_columns(
            list(summary_df_all.columns), summary_df_all.values.tolist(), SUMMARY_VIEW_COLUMNS.get(view_mode)
        )
        ...
        if summary_search.strip():
            mask = summary_df.astype(str).apply(lambda col: col.str.lower().str.contains(summary_search.strip().lower(), na=False))
            summary_df = summary_df[mask.any(axis=1)]
        st.dataframe(to_dataframe(summary_df.columns.tolist(), summary_df.values.tolist()), use_container_width=True, hide_index=True)
        st.download_button(
            "Download summary CSV",
            data=df_to_csv_bytes(to_dataframe(summary_df.columns.tolist(), summary_df.values.tolist())),
            file_name=f"openrem_rf_physician_summary_view_{view_mode}.csv",
            mime="text/csv",
        )

    with tabs[1]:
        accession_search = st.text_input("Search accession number(s)", value="")
        st.caption("Use comma, semicolon, space, or new line for multiple accession numbers.")

        event_headers, event_rows = filter_columns(
            list(events_df_all.columns), events_df_all.values.tolist(), EVENT_VIEW_COLUMNS.get(view_mode)
        )
        ...
        visible_df = filter_events_by_accession(visible_df, accession_search)

        selected_ids = []
        if "irradiation_event_id" in visible_df.columns:
            selected_ids = st.multiselect(
                "Optional: build summary from selected visible event IDs",
                options=visible_df["irradiation_event_id"].tolist(),
                default=[],
            )

        basis_df = visible_df.copy()
        if selected_ids and "irradiation_event_id" in visible_df.columns:
            basis_df = visible_df[visible_df["irradiation_event_id"].isin(selected_ids)].copy()
            st.info(f"Summary basis: {len(basis_df)} selected rows")
        else:
        ...
        if not rebuilt_summary_df.empty:
            ...
            st.dataframe(to_dataframe(rebuilt_summary_df.columns.tolist(), rebuilt_summary_df.values.tolist()), use_container_width=True, hide_index=True)

        st.dataframe(style_events(visible_df), use_container_width=True, hide_index=True)
        st.download_button(
            "Download event rows CSV",
            data=df_to_csv_bytes(to_dataframe(visible_df.columns.tolist(), visible_df.values.tolist())),
            file_name=f"openrem_rf_event_rows_view_{view_mode}.csv",
            mime="text/csv",
        )
